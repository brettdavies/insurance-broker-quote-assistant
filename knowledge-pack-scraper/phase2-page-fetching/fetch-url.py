...
async def process_result(
    ...
    # Extract first 2 chars of ID for sharding (page_a0xyz... → a0/)
    prefix = page_id.split('_')[1][:2]  # First 2 chars after 'page_'
    output_dir = tm.output_base / 'pages' / prefix
    ...
    # Update url-tracker (with sharded paths)
    tm.update_status('url', url_id, 'completed', {
        'pageId': page_id,
        'htmlFile': f"../raw/pages/{prefix}/{page_id}.html",
        'markdownFile': f"../raw/pages/{prefix}/{page_id}.md",
        'fetchedAt': datetime.now().isoformat()
    })

    # Register page in page-tracker (with sharded paths)
