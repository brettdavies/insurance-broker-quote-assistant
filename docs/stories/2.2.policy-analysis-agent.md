...
- [x] **Task 1: Create Policy Analysis Service** (AC: 3, 4, 5, 6, 7, 8)
  ...
  - [ ] Implement `analyzePolicy()` function that accepts `policySummary: PolicySummary` and `policyText?: string`
  - [ ] Accept `llmProvider: LLMProvider` and `knowledgePackRAG: KnowledgePackRAG` as constructor parameters (dependency injection)
  - [ ] Query knowledge pack RAG for carrier discounts using `getCarrierByName()` and `getCarrierDiscounts()`
  - [ ] Query knowledge pack RAG for bundle opportunities using carrier product list
  - [ ] Build LLM prompt with policy data + knowledge pack context (discounts, bundle rules, pricing data)
  - [ ] Use Gemini 2.5 Flash-Lite with structured outputs (JSON Schema) for analysis
  - [ ] Extract three opportunity types: missing discounts, bundle options, deductible trade-offs
  - [ ] Include cuid2-based citations for each recommendation (from knowledge pack queries)
  - [ ] Rank opportunities by estimated annual savings impact (high to low)
  - [ ] Return `PolicyAnalysisResult` with opportunities array, bundle options, deductible optimizations
  - [ ] Log token usage for each LLM analysis call
...
- [x] **Task 2: Create PolicyAnalysisResult Schema** (AC: 7, 8)
  ...
  - [ ] Define structure: `currentPolicy` (PolicySummary), `opportunities` (Opportunity[]), `bundleOptions` (BundleOption[]), `deductibleOptimizations` (DeductibleOptimization[]), `pitch` (string), `complianceValidated` (boolean), `trace` (DecisionTrace)
  - [ ] Create `BundleOption` schema: `product` (string), `estimatedSavings` (number), `requiredActions` (string[]), `citation` (Citation object)
  - [ ] Create `DeductibleOptimization` schema: `currentDeductible` (number), `suggestedDeductible` (number), `estimatedSavings` (number), `premiumImpact` (number), `citation` (Citation object)
  - [ ] Reuse `Opportunity` schema from `@repo/shared` (already defined in intake flow)
...
- [x] **Task 3: Implement Knowledge Pack Query Helpers** (AC: 4, 5, 8)
  ...
  - [ ] Add `getCarrierDiscounts(carrierName: string, stateCode: string, productType: string)` function
  - [ ] Returns array of discount objects with cuid2 IDs, requirements, percentages, citations
  - [ ] Add `getCarrierBundleDiscounts(carrierName: string, stateCode: string)` function
  - [ ] Returns bundle discount rules (e.g., "auto + home = 15% off both")
  - [ ] Add `getCarrierProducts(carrierName: string)` function (may already exist, verify)
  - [ ] All functions return results with cuid2-based citations for audit trail
  - [ ] Handle missing carrier/state data gracefully (return empty arrays)
...
- [x] **Task 4: Create POST /api/policy/analyze Endpoint** (AC: 1, 2, 7, 9, 11)
  ...
  - [ ] Accept request body: `{ policyText: string, policySummary?: PolicySummary }`
  - [ ] Use Zod validator middleware for request validation
  - [ ] If `policySummary` not provided, extract from `policyText` using Conversational Extractor (reuse `extractPolicyData()`)
  - [ ] Call Policy Analysis Agent service with extracted PolicySummary
  - [ ] Call Pitch Generator Agent to create savings pitch from opportunities (reuse from intake flow)
  - [ ] Call Compliance Filter to validate pitch (reuse from intake flow)
  - [ ] Create decision trace with: inputs (policyText, policySummary), extraction results, analysis results (opportunities with citations), LLM calls (tokens), outputs (PolicyAnalysisResult)
  - [ ] Log decision trace to compliance log
  - [ ] Return `PolicyAnalysisResult` response
...
- [x] **Task 5: Implement LLM Analysis Prompt Engineering** (AC: 3, 4, 5, 6, 10)
  - [ ] Design prompt template in `apps/api/src/services/policy-analysis-agent.ts`
  - [ ] Include policy context: carrier, state, product type, coverage limits, deductibles, premiums
  - [ ] Include knowledge pack context: available discounts, bundle rules, pricing data (with citations)
  - [ ] Instruct LLM to identify: missing discounts, bundle opportunities, deductible trade-offs
  - [ ] Require LLM to cite knowledge pack cuid2 IDs for each recommendation
  - [ ] Require LLM to calculate estimated annual savings in dollars
  - [ ] Use structured output schema (JSON Schema) to enforce response format
  - [ ] Validate LLM response against PolicyAnalysisResult schema before returning
  - [ ] Handle LLM hallucinations (filter out recommendations without valid citations)
...
- [x] **Task 6: Integrate with Pitch Generator Agent** (AC: 7, 10)
  ...
  - [ ] Pass opportunities, bundle options, deductible optimizations to pitch generator
  - [ ] Include policy context (current carrier, premium) in pitch generation
  - [ ] Ensure pitch includes "because" rationales for each recommendation (spec requirement)
  - [ ] Validate pitch clarity (readability score ≥85% in evaluation harness)
  - [ ] Log token usage for pitch generation

- [x] **Task 7: Integrate with Compliance Filter** (AC: 11)
  - [ ] Reuse Compliance Filter from conversational intake flow (`apps/api/src/services/compliance-filter.ts`)
  - [ ] Validate generated pitch against prohibited phrases
  - [ ] Inject required disclaimers into pitch
  - [ ] Set `complianceValidated` field in PolicyAnalysisResult based on filter result
  - [ ] Log compliance violations to compliance log if detected

- [x] **Task 8: Implement Deductible/Limit Trade-off Analysis** (AC: 6, 8)
  - [ ] In Policy Analysis Agent, analyze current deductibles against knowledge pack pricing data
  - [ ] Calculate premium impact of deductible changes (e.g., $500→$1000 saves $200/yr)
  - [ ] Consider coverage limit adjustments (e.g., increasing liability limits)
  - [ ] Include knowledge pack citations for pricing data used in calculations
  - [ ] Return `DeductibleOptimization[]` with current vs suggested values, savings, premium impact
  - [ ] Rank by annual savings impact (highest first)

- [x] **Task 9: Implement Bundle Opportunity Detection** (AC: 5, 8)
  - [ ] In Policy Analysis Agent, check if policy has single product (auto OR home, not both)
  - [ ] Query knowledge pack for bundle discounts offered by current carrier
  - [ ] Identify missing products that would qualify for bundle savings (e.g., "Add home to auto for 15% off both")
  - [ ] Calculate estimated bundle savings based on knowledge pack pricing
  - [ ] Include required actions (e.g., "Add home insurance policy")
  - [ ] Return `BundleOption[]` with product, estimated savings, required actions, citations
  - [ ] Cross-check carrier availability for recommended products in client's state

- [x] **Task 10: Add Decision Trace Logging** (AC: 11)
  - [ ] Extend `apps/api/src/utils/decision-trace.ts` to support policy analysis flow
  - [ ] Create decision trace with flow type `'policy'`
  - [ ] Log inputs: policyText, policySummary
  - [ ] Log extraction results: PolicySummary (if extracted from text)
  - [ ] Log analysis results: opportunities with citations, bundle options, deductible optimizations
  - [ ] Log LLM calls: model (gemini-2.5-flash-lite), tokens used, analysis time
  - [ ] Log knowledge pack queries: carrier discounts queried, bundle rules consulted (with cuid2 IDs)
  - [ ] Log outputs: PolicyAnalysisResult with pitch
  - [ ] Write to compliance log file (`logs/compliance.log`)

- [x] **Task 11: Add Error Handling** (AC: 1, 2)
  - [ ] Handle invalid policy text: Return `VALIDATION_ERROR` if policyText is empty or malformed
  - [ ] Handle extraction failures: Return `EXTRACTION_FAILED` if PolicySummary extraction fails
  - [ ] Handle LLM analysis failures: Return partial results with low-confidence opportunities, log error
  - [ ] Handle knowledge pack query failures: Return empty opportunities array, log warning
  - [ ] Handle missing carrier data: Return `KNOWLEDGE_PACK_ERROR` if carrier not found in knowledge pack
  - [ ] Use global error handler middleware for consistent error responses
  - [ ] Log errors to program log with full context
...
- [x] **Task 12: Add Unit Tests** (AC: 7, 8, 9)
  ...
  - [ ] Test policy analysis with mock PolicySummary and knowledge pack data
  - [ ] Test discount identification (missing discounts detected correctly)
  - [ ] Test bundle opportunity detection (single product → bundle recommendation)
  - [ ] Test deductible optimization calculation (savings calculated correctly)
  - [ ] Test citation inclusion (all opportunities have cuid2 citations)
  - [ ] Test opportunity ranking (sorted by annual savings impact)
  - [ ] Mock LLM provider for deterministic tests
  - [ ] Mock knowledge pack RAG for test data

- [x] **Task 13: Add Integration Tests** (AC: 1, 2, 3, 9, 11)
  - [x] Create `apps/api/src/routes/__tests__/policy-analyze.test.ts`
  - [ ] Test full policy analysis flow: policy text → extraction → analysis → pitch → compliance
  - [ ] Test with real knowledge pack data (loaded at test startup)
