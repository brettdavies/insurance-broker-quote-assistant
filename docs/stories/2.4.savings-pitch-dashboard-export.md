...
- [ ] **Task 1: Create Savings Dashboard Component** (AC: 1, 2, 3, 4)
  - [ ] Create `apps/web/src/components/sidebar/SavingsDashboard.tsx` component
  - [ ] Accept `PolicyAnalysisResult` as prop from parent component
  - [ ] Group opportunities into three categories: Discounts (from `opportunities` array), Bundles (from `bundleOptions` array), Coverage Adjustments (from `deductibleOptimizations` array)
  - [ ] Display each opportunity card with: discount/opportunity name, estimated savings (dollar amount), eligibility confidence score (0-100), required actions array, citation tooltip on hover
  - [ ] Implement visual prioritization: High-impact (green `#22c55e`) for savings >$200/yr, Medium-impact (yellow `#eab308`) for savings $50-$200/yr, Low-impact (gray `#9ca3af`) for savings <$50/yr
  - [ ] Use shadcn/ui Card component for opportunity cards with border colors matching priority
  - [ ] Add citation tooltip showing cuid2 ID and source file on hover over citation icon
  - [ ] Display confidence score as percentage badge (e.g., "85% confidence")
  - [ ] Show required actions as bullet list under each opportunity
  - [ ] Add collapsible accordion sections for each category (Discounts, Bundles, Coverage Adjustments)
  - [ ] Position dashboard in sidebar (replaces Routing Status panel in policy mode)

- [ ] **Task 2: Implement Export Functionality** (AC: 5, 9, 10)
  - [ ] Create `apps/web/src/lib/export-utils.ts` utility file
  - [ ] Implement `exportSavingsPitch()` function that accepts `PolicyAnalysisResult` and generates JSON export
  - [ ] Auto-generate filename using pattern: `savings_pitch_{name}_{state}_{product}_{date}.json` (e.g., `savings_pitch_jane_doe_CA_home_20251106.json`)
  - [ ] Extract name from `currentPolicy` if available, otherwise use "client"
  - [ ] Format date as YYYYMMDD (e.g., `20251106`)
  - [ ] Validate export data using `policyAnalysisResultSchema` from `@repo/shared` before export
  - [ ] Create downloadable JSON file using browser `Blob` API and `URL.createObjectURL()`
  - [ ] Trigger download via `<a>` element with `download` attribute
  - [ ] Clean up object URL after download completes
  - [ ] Handle errors gracefully with toast notification if export fails

- [ ] **Task 3: Implement Copy to Clipboard Functionality** (AC: 6)
  - [ ] Create `apps/web/src/lib/clipboard-utils.ts` utility file
  - [ ] Implement `copySavingsPitchToClipboard()` function that accepts `PolicyAnalysisResult`
  - [ ] Format pitch text for client email: Include current policy summary, grouped opportunities by category, compliance disclaimers, and citations
  - [ ] Use markdown formatting for readability (headers, bullet lists, emphasis)
  - [ ] Include adaptive compliance disclaimers from `complianceValidated` flag and state/product context
  - [ ] Use browser `navigator.clipboard.writeText()` API
  - [ ] Show toast notification on successful copy: "Savings pitch copied to clipboard"
  - [ ] Handle clipboard API errors gracefully (fallback to manual copy if clipboard API unavailable)

- [ ] **Task 4: Integrate Slash Commands** (AC: 5, 6, 7)
  - [ ] Add `/export` command handler to global slash command system (see `keyboard-shortcuts-reference.md`)
  - [ ] Add `/copy` command handler to global slash command system
  - [ ] Ensure commands work globally from anywhere in app (notes input, sidebar, dashboard)
  - [ ] Execute `exportSavingsPitch()` when `/export` command triggered
  - [ ] Execute `copySavingsPitchToClipboard()` when `/copy` command triggered
  - [ ] Show visual feedback (toast notification) when command executes
  - [ ] Update keyboard shortcuts help modal (`/help`) to include `/export` and `/copy` commands

- [ ] **Task 5: Add Compliance Disclaimers to Export/Copy** (AC: 8)
  - [ ] Extract state and product from `currentPolicy` in `PolicyAnalysisResult`
  - [ ] Load adaptive compliance disclaimers based on state/product combination
  - [ ] Include disclaimers in both JSON export and clipboard copy formats
  - [ ] Format disclaimers as separate section in export/copy output
  - [ ] Use compliance disclaimers from knowledge pack or backend compliance filter (see architecture docs)
  - [ ] Ensure disclaimers appear at end of pitch text for maximum visibility

- [ ] **Task 6: Integrate Dashboard into Policy Analysis Flow** (AC: 1)
  - [ ] Update `apps/web/src/components/policy/PolicyAnalysisView.tsx` (or equivalent component) to render SavingsDashboard
  - [ ] Pass `PolicyAnalysisResult` from TanStack Query mutation result to SavingsDashboard component
  - [ ] Show dashboard only when `PolicyAnalysisResult` is available (after analysis completes)
  - [ ] Display loading state while analysis in progress (use TanStack Query `isLoading` state)
  - [ ] Handle error states gracefully (show error message if analysis fails)
  - [ ] Position dashboard in sidebar (30% width, resizable) replacing Routing Status panel

- [ ] **Task 7: Add Decision Trace Logging** (AC: 11)
  - [ ] Log export action to decision trace when `/export` command executed
  - [ ] Log copy action to decision trace when `/copy` command executed
  - [ ] Include timestamp, action type, and PolicyAnalysisResult summary in trace
  - [ ] Use existing decision trace logging infrastructure from backend (see `docs/architecture/4-data-models.md#48-decisiontrace`)
  - [ ] Note: Frontend logs to console for now (backend handles compliance log file writing)

- [ ] **Task 8: Unit Tests** (AC: 10)
  - [ ] Create `apps/web/src/lib/__tests__/export-utils.test.ts`
  - [ ] Test `exportSavingsPitch()` with valid `PolicyAnalysisResult`
  - [ ] Test filename generation with various inputs (with/without name, different states/products)
  - [ ] Test Zod schema validation catches invalid data
  - [ ] Create `apps/web/src/lib/__tests__/clipboard-utils.test.ts`
  - [ ] Test `copySavingsPitchToClipboard()` formatting (markdown structure, disclaimers included)
  - [ ] Mock `navigator.clipboard.writeText()` for testing
  - [ ] Create `apps/web/src/components/sidebar/__tests__/SavingsDashboard.test.tsx`
  - [ ] Test dashboard renders all three categories (Discounts, Bundles, Coverage Adjustments)
  - [ ] Test visual prioritization (green/yellow/gray based on savings amount)
  - [ ] Test citation tooltips display correctly
  - [ ] Test confidence score badges render correctly
