...
- [x] **Task 1: Create PrefillPacket Schema** (AC: 1, 2, 3, 4, 5, 10)
  ...
  - [x] Define `PrefillPacket` schema with fields:
    ...
    - [x] **Quote Essentials**: `state` (string, required), `productType` (string enum, required), `carrier` (string, optional - primary carrier from routing)
...
- [x] **Task 2: Create Prefill Generator Service** (AC: 1, 2, 3, 4, 5)
  ...
  - [x] Map UserProfile fields to PrefillPacket quote essentials:
    ...
    - [x] `profile.productType` → `prefill.productType` (required)
    - [x] `route.primaryCarrier` → `prefill.carrier`
  - [x] Map UserProfile fields to product-specific data based on `productType`:
  ...
  - [x] Handle edge cases: missing state/productType (should not happen but handle gracefully), empty missingFields array
...
- [x] **Task 4: Implement Missing Fields Detection** (AC: 2, 3)
  ...
  - [x] Define required fields per product type:
    - [x] Auto: `state`, `productType`, `vehicles` (critical), `drivers` (critical), `vins` (important), `garage` (optional)
    - [x] Home: `state`, `productType`, `propertyType` (critical), `yearBuilt` (important), `squareFeet` (important), `roofType` (optional)
    - [x] Renters: `state`, `productType`, `propertyType` (critical), `personalProperty` (important)
    - [x] Umbrella: `state`, `productType`, `existingPolicies` (critical)
...
- [x] **Task 7: Implement Frontend Export Functionality** (AC: 6, 7)
  ...
  - [x] Create `handleExport()` function that:
    ...
    - [x] Generates filename: `prefill_{name}_{state}_{productType}_{YYYYMMDD}.json` with sanitization rules:
...
- [x] **Task 10: Add Unit Tests** (AC: 10)
  ...
  - [x] Test edge cases:
    - [x] Missing state/productType (should handle gracefully)
...
- **PrefillPacketStub schema**: Currently stub schema in `packages/shared/src/schemas/intake-result.ts` with `state`, `productType`, `carrier`, `disclaimers` fields - needs full implementation [Source: packages/shared/src/schemas/intake-result.ts:65-72]
...
- **UserProfile**: Schema includes all fields needed for prefill packet mapping (name, email, phone, state, productType, vehicles, drivers, etc.) [Source: packages/shared/src/schemas/user-profile.ts]
...
- **Export filename**: Auto-generate filename from prefill packet data: `prefill_{name}_{state}_{productType}_{YYYYMMDD}.json` - sanitize name (remove spaces, special characters except underscores, convert to lowercase, limit to 50 chars), use current date in YYYYMMDD format [Source: epic requirements AC7]
...
- **Given** a UserProfile with state and productType
...
- **Then** file is downloaded with format `prefill_{name}_{state}_{productType}_{YYYYMMDD}.json`
