...
- [ ] **Task 3: Create Adaptive Sidebar Component** (AC: 2, 3, 4)
  ...
  - [ ] Each field displays: name, value, confidence score (if LLM-extracted), info icon (ℹ️) with tooltip
  - [ ] Create `MissingFields.tsx` component with priority indicators
  - [ ] Priority indicators: 🔴 Red (critical), 🟡 Yellow (important), ⚪ Gray (optional)
...
- [ ] **Task 14: Add Integration Tests** (AC: 4, 9)
  ...
  - [ ] Test end-to-end flow: Type message → Extract fields → Update sidebar
...
- **UserProfile**: Schema defined in `packages/shared/src/schemas/user-profile.ts` with fields: `state`, `productType`, `age`, `householdSize`, `vehicles`, `ownsHome`, `cleanRecord3Yr`, `existingPolicies` (array for bundle analysis) [Source: architecture/4-data-models.md#42-userprofile]
...
- **Priority Indicators**: 🔴 Red circle (critical), 🟡 Yellow circle (important), ⚪ Gray circle (optional) [Source: front-end-spec.md#enhanced-missing-fields-component-specification]
...
- **Toast Notifications**: Bottom-right corner, stack upward, auto-dismiss 5 seconds, compact design with ⓘ icon and ✕ close button [Source: front-end-spec.md#toast-notification-component]
...
- **Missing Fields Detection**: Priority indicators (🔴 critical, 🟡 important, ⚪ optional), progress bar with completion percentage
...
✅ **AC1:** Chat panel displays conversation history with clear visual distinction (dark mode default)  
✅ **AC2:** Right sidebar shows live captured fields organized by category (Identity, Location, Product, Details)  
✅ **AC3:** Missing required fields highlighted with red indicator and field name  
✅ **AC4:** Captured fields update in real-time with optimistic UI updates  
✅ **AC5:** Slash command shortcuts implemented (field shortcuts + action shortcuts + help)  
✅ **AC6:** Direct key-value syntax support (`kids:3`, `k:3`, `deps:4`, `car:garage`)  
✅ **AC7:** Field-specific modals open from shortcuts, inject formatted key-value pairs  
✅ **AC8:** Keyboard shortcuts reference documented  
✅ **AC9:** TanStack Query manages chat state with automatic refetching and cache invalidation  
✅ **AC10:** Adaptive compliance disclaimers displayed based on discovered state/product  
✅ **AC11:** No auto-generated broker prompts - broker controls conversation  
✅ **AC12:** Professional dark mode styling with monospace font for data fields  
✅ **AC13:** Key-value pairs visually distinct with syntax highlighting (green/red/yellow pills)  
✅ **AC14:** Chat panel displays ONLY broker input - AI extraction is invisible
