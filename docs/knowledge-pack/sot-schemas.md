...
### Discount Requirements Structure (Extended)

**Purpose:** The `requirements.value` object supports both product-based and household-based qualifiers to enable granular discount eligibility rules.

**All Requirement Fields Are Optional:** Discounts can use any combination of the following qualifiers. ALL specified qualifiers must pass for discount eligibility (AND logic).

**Product-Based Qualifiers:**
- `mustHaveProducts` - Array of product types required (e.g., `["auto", "home"]` for multi-policy bundle)
- `minProducts` - Minimum number of products required (e.g., `2` for bundle discount)

**Household-Based Qualifiers:**
- `minChildren` - Minimum number of children (e.g., `2` for family plan discount)
- `maxChildren` - Maximum number of children for eligibility
- `minDependents` - Minimum number of dependents
- `minVehicles` - Minimum vehicles for multi-car discount (e.g., `2`)
- `maxVehicles` - Maximum vehicles for eligibility
- `garageType` - Array of acceptable garage types (e.g., `["garage", "carport"]` for security discount)

**Property-Based Qualifiers:**
- `propertyRequirements` - Object with property-specific rules:
  - `roofType` - Array of qualifying roof types (e.g., `["metal", "tile"]` for wind-resistant discount in hurricane zones)
  - `minConstructionYear` - Minimum year built (e.g., `2000` for new home discount)
  - `propertyTypes` - Array of qualifying property types (e.g., `["single-family"]` to exclude condos)

**Demographic Qualifiers:**
- `ageRange` - Object with `min` and `max` age (e.g., `{min: 55, max: 75}` for mature driver discount)

**Driving Record Qualifiers:**
- `drivingRequirements` - Object with driving history rules:
  - `cleanRecord` - Boolean requiring clean driving record
  - `minYearsClean` - Minimum years without violations (e.g., `3` for safe driver discount)
  - `maxViolations` - Maximum allowed violations

**Real-World Discount Examples:**
- **Family Discount:** `{ minChildren: 2 }` - GEICO/Progressive offer discounts for families with 2+ kids
- **Multi-Car Discount:** `{ minVehicles: 2 }` - Standard multi-car discount across carriers
- **Garage Discount:** `{ garageType: ["garage"] }` - Vehicles stored in enclosed garage reduce theft/damage risk
- **New Home Discount:** `{ propertyRequirements: { minConstructionYear: 2010 } }` - Newer construction = lower risk
- **Wind-Resistant Roof Discount:** `{ propertyRequirements: { roofType: ["metal", "tile"] } }` - Hurricane-prone states (FL, LA, TX)
- **Mature Driver Discount:** `{ ageRange: { min: 55, max: 75 } }` - Experienced drivers with established safe driving patterns
- **Safe Driver Discount:** `{ drivingRequirements: { cleanRecord: true, minYearsClean: 3 } }` - No violations for 3+ years

**Source Citation Requirement:**
Each qualifier should have `_sources` array pointing to where requirement was documented (e.g., carrier website discount FAQ, underwriting guidelines PDF).

**Evaluation Logic:**
ALL specified qualifiers must pass (AND logic). For example:
```json
{
...
```
This requires: Auto product AND 2+ vehicles AND garage storage. Missing any qualifier = discount not eligible.

**Alignment with Eligibility Schema:**
The eligibility schema (Section 1, lines 198-212) already supports `minAge`, `maxAge`, `maxVehicles` for carrier-level underwriting rules. Discount requirements now have parallel capabilities for discount-level qualifications, ensuring consistency across schemas.
