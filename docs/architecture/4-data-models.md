...
| **ExtractionResult**     | `packages/shared/src/schemas/` | Known/inferred/suppressed field separation         | Part of IntakeResult.extraction           |
...
**Metadata Fields (As-Built):**

- `_inferred` - Record<string, unknown> - Inferred field values (confidence <85%, user can dismiss)
- `_suppressed` - string[] - User-dismissed fields (never re-inferred)
...
- **Known/Inferred/Suppressed metadata:** `_inferred` and `_suppressed` fields enable three-tier field classification system (known ≥85% confidence, inferred <85%, suppressed = dismissed by user)
...
## 4.6 ExtractionResult

**Purpose:** Separates known vs inferred fields from extraction, enabling three-tier field classification system.

**Schema Location:** `packages/shared/src/schemas/intake-result.ts` (exported as `extractionResultSchema`)

**Key Fields:**

- `method` - Extraction method used (`'key-value' | 'llm'`)
- `known` - Partial<UserProfile> - Known fields (confidence ≥85% or broker-set, read-only to LLM)
- `inferred` - Partial<UserProfile> - Inferred fields (confidence <85%, user can dismiss)
- `suppressedFields` - string[] - User-dismissed fields (never re-inferred)
- `confidence` - Record<string, number> - Per-field confidence scores (0-1)
...
- `tokenUsage` - TokenUsage (optional) - LLM token usage if method === 'llm'

**Design Decisions:**

- **Three-tier classification:** Known (≥85%), Inferred (<85%), Suppressed (dismissed)
- **Extraction method tracking:** Enables transparency about deterministic vs LLM extraction
- **Confidence scores:** Per-field confidence enables threshold-based separation
- **Inference reasons:** Provides transparency for broker review
- **Suppression list:** Prevents re-inferring dismissed fields (broker has final say)

**Architecture Context:**

Part of the hybrid extraction architecture (Section 6.1). ExtractionResult is included in IntakeResult.extraction field, replacing the deprecated `profile` field for new clients.
...
- `disclaimers` - Array of required disclaimers (state/product-specific)
...
extraction: {
  ...
  suppressedFields?: string[],            // Fields broker dismissed (never re-inferred)
  inferenceReasons?: Record<string, string>, // Reasoning for each inferred field
