  ...
  if (line.trim() === "```") {
    if (inCodeBlock) {
      ...
      for (const query of codeBlockContent) {
        ...
        if (trimmed && !trimmed.startsWith("#") && !trimmed.startsWith("site:") &&
            !trimmed.startsWith("intitle:") && !trimmed.startsWith("filetype:") &&
            !trimmed.includes("Example:") && trimmed.length > 5) {
...
function determinePriority(section: string, hasCarrier: boolean): "high" | "medium" | "low" {
  if (section.includes("Carrier Operating States") || section.includes("Auto Insurance Discounts")) {
    return "high";
  }
...
queries.forEach((q, index) => {
  ...
  categoriesMap.get(categoryInfo.name)!.searches.push(search);
});
